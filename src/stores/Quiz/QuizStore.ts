        ...
        persist(
            (set, get) => ({
                ...
                toggleQuizSelection: (quizId: string) => {
                    ...
                    if (!quiz) {
                        console.warn(
                            `Quiz with id ${quizId} not found in current series`,
                        );
                        return;
                    }
                ...
                selectQuizzes: (quizIds: string[]) => {
                    ...
                    if (!state.currentSeries) {
                        console.warn(
                            "No current series available for quiz selection",
                        );
                        return;
                    }

                    // Lấy danh sách all quizIds có thể chọn từ current series
                    const availableQuizIds = state.currentSeries.quizzes.map(
                        (q) => q.id,
                    );
                    ...
                    if (invalidIds.length > 0) {
                        console.warn(
                            `Invalid quiz IDs ignored: ${invalidIds.join(", ")}`,
                        );
                    }
                    ...
                    console.log(
                        `Selected ${validQuizIds.length} quizzes:`,
                        validQuizIds,
                    );
                },
                ...
                setCurrentQuiz: (quizId: string) => {
                    ...
                    if (!state.selectedQuizIds.includes(quizId)) {
                        console.warn(
                            `Quiz ${quizId} is not in selected quizzes list`,
                        );
                        return;
                    }
                ...
                navigateToNextQuiz: () => {
                    ...
                    if (!navInfo.nextQuizId) {
                        console.warn(
                            "Cannot navigate to next quiz: nextQuizId is null",
                        );
                        return;
                    }
                    ...
                    console.log(
                        `Navigated to next quiz: ${navInfo.nextQuizId}`,
                    );
                },
                ...
                navigateToPreviousQuiz: () => {
                    ...
                    console.log(
                        `Navigated to previous quiz: ${navInfo.prevQuizId}`,
                    );
                },
                ...
                getNavigationInfo: (): NavigationInfo => {
                    ...
                    if (
                        !state.currentSeries ||
                        state.selectedQuizIds.length === 0
                    ) {
                        return {
                            currentIndex: -1,
                            totalSelected: 0,
                ...
                updateAnswer: (
                    ...
                    if (existingIndex >= 0) {
                        ...
                        const newAnswer = createUserAnswer(
                            questionId,
                            selectedOptions,
                        );
                ...
                validateSubmission: (): SubmissionValidation => {
                    ...
                    if (!state.currentSeries) {
                        return {
                            canSubmit: false,
                            missingRequirements: [
                                "Không tìm thấy bộ quiz để submit",
                            ],
                            completedQuizzes: [],
                            incompleteQuizzes: [],
                        };
                    ...
                    if (state.selectedQuizIds.length === 0) {
                        return {
                            canSubmit: false,
                            missingRequirements: [
                                "Vui lòng chọn ít nhất một quiz để submit",
                            ],
                            completedQuizzes: [],
                            incompleteQuizzes: [],
                        };
                    ...
                    if (!canSubmit && missingRequirements.length === 0) {
                        missingRequirements.push(
                            "Vui lòng hoàn thành tất cả quiz đã chọn",
                        );
                    }
                ...
                canNavigateToQuiz: (quizId: string): boolean => {
                    ...
                    if (!state.currentSeries) {
                        console.warn(
                            "No current series available for navigation check",
                        );
                        return false;
                    }
                    ...
                    if (!quiz) {
                        console.warn(
                            `Quiz ${quizId} not found in current series`,
                        );
                        return false;
                    }
                    ...
                    if (!state.selectedQuizIds.includes(quizId)) {
                        console.warn(
                            `Quiz ${quizId} is not selected - cannot navigate`,
                        );
                        return false;
                    }
                ...
                updateQuizStatus: (quizId: string) => {
                    ...
                    const hasCompletionTime =
                        !!state.quizCompletionTimes[quizId];
                    ...
                    if (isNowCompleted) {
                        if (!hasCompletionTime) {
                            ...
                            console.log(
                                `Quiz ${quizId} completed for first time`,
                            );
                        } else if (!wasCompleted) {
                            // Re-completing after becoming incomplete - update completion time
                            get().markQuizCompletionTime(quizId);
                            console.log(
                                `Quiz ${quizId} re-completed - time updated`,
                            );
                        }
                    ...
                    console.log(
                        `Quiz ${quizId} status: ${oldStatus} → ${newStatus}`,
                    );
                },
                ...
                getQuizById: (quizId: string): Quiz | null => {
                    ...
                    const state = get();
                    return (
                        state.currentSeries?.quizzes.find(
                            (q) => q.id === quizId,
                        ) || null
                    );
                },
                ...
                getQuestionById: (
                    ...
                    return (
                        quiz?.questions.find((q) => q.id === questionId) || null
                    );
                },
