"use server"
import { PracticeTestCodeCheckResponse, PracticeTestLanguageSelectsResponse, PracticeTestSelectResponse, PracticeTestSelectsResponse, PracticeTestUserTemplateCodeSelectResponse } from "EduSmart/api/api-quiz-service";
...
export async function selectPracticeTestDetailList(ProblemId: string): Promise<PracticeTestSelectResponse | null> {
  try {
    const resp = await quizService.api.v1PracticeTestSelectPracticeTestDetailList({
      ProblemId: ProblemId,
    });
...
export async function selectUserTemplateCodeList(
  ProblemId: string, LanguageId: number
...
export async function checkPracticeTestCodeCreate(
  ProblemId: string, LanguageId: number, sourceCode: string, inputs: string[],
): Promise<PracticeTestCodeCheckResponse | null> {
  try {
    const resp = await quizService.api.v1PracticeTestCheckPracticeTestCodeCreate({
      problemId: ProblemId,
      languageId: LanguageId,
