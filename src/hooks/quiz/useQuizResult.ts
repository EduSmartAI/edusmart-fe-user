    ...
    const transformTestResult = useCallback(
        (testResult: StudentTestResult): QuizResultData[] => {
            ...
            return testResult.quizzesResults.map((quizResult) => {
                const questions = quizResult.questionsResult.map(
                    (question) => ({
                        id: question.questionId,
                        text: question.questionText,
                        type: QuestionType.SINGLE_CHOICE, // Default, could be derived from questionType field
                ...
                const userAnswers: QuizAnswer[] =
                    quizResult.questionsResult.map((question) => {
