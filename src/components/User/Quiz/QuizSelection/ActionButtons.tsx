        ...
        <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
          ...
          <div className="text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start space-x-2">
              <FiCheckCircle className={`w-5 h-5 ${selectedCount > 0 ? 'text-green-500' : 'text-gray-400'}`} />
              <span className="text-lg font-medium text-gray-900 dark:text-white">
                Đã chọn{" "}
                <span className={`font-bold ${selectedCount > 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}`}>
                  {selectedCount}
                </span>{" "}
                bài quiz
              </span>
            ...
            <Button
              ...
              className={`px-8 py-3 h-auto font-semibold rounded-xl shadow-lg transition-all duration-300 ${
                disabled || selectedCount === 0
                  ...
                  : 'bg-gradient-to-r from-green-600 to-blue-600 border-none hover:from-green-700 hover:to-blue-700 hover:shadow-xl hover:scale-105'
              }`}
