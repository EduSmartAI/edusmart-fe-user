...
const QuizTakingScreenNew: React.FC<QuizTakingScreenNewProps> = ({
    ...
    }, [state.timeRemaining, handleSubmitTest, actions]);
    ...
    // No test data
    if (!state.testDetail || state.testDetail.quizzes.length === 0) {
        return (
            <Layout className="min-h-screen bg-gray-50 dark:bg-gray-900">
                <Content className="p-6 flex items-center justify-center">
                    <Alert
    ...
    const totalQuestions = state.testDetail.quizzes.reduce(
        (total, quiz) => total + quiz.questions.length,
        0,
    );
    const currentQuestionNumber =
        state.testDetail.quizzes
            .slice(0, state.currentQuizIndex)
    ...
    const canGoPrevious = () => {
        return !(
            state.currentQuizIndex === 0 && state.currentQuestionIndex === 0
        );
    };
