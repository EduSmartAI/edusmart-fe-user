import { selectCodeLangues, selectPracticeTestDetailList, selectPracticeTestsList } from "EduSmart/app/(codeQuiz)/action";
import CodeEditor, { SubmitPayload } from "../../components/Code/CodeEditor";
import {
...
async function loadCodeEditorData(): Promise<{
...
type CodeEditorContainerProps = {
  onSubmit?: (payload: SubmitPayload) => void;
};

export default async function CodeEditorContainer({
  onSubmit,
}: CodeEditorContainerProps) {
  const { languages, problems } = await loadCodeEditorData();

  return <CodeEditor languages={languages} problems={problems} onSubmit={onSubmit} />;
}
